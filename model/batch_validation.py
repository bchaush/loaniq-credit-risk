...
def validate_and_prepare_batch(
    ...
    if frame is None or not isinstance(frame, pd.DataFrame):
        return BatchValidationResult(ok=False, errors=["Upload is not a tabular dataframe."])
    ...
    if working.empty:
        return BatchValidationResult(ok=False, errors=["CSV contains no applicant rows."])
    if len(working) > 5000:
    ...
    for col in candidate_numeric:
        for idx, raw in working[col].items():
            if _is_missing(raw) or (isinstance(raw, str) and raw.strip() == ""):
    ...
    # Required fields must be finite after conversion.
    ...
    for idx, row in working.iterrows():
        ...
        if not _is_missing(ext2) and not _is_missing(ext3):
            ...
            if expected_sum is not None:
                if "ext_score_sum" not in working.columns or _is_missing(row.get("ext_score_sum")):
        ...
        if not _is_missing(ext2):
            expected_flag = derive_low_ext_score_2(ext2)
            if "low_ext_score_2" in working.columns and not _is_missing(row.get("low_ext_score_2")):
                if int(float(row.get("low_ext_score_2"))) != expected_flag:
                    add_error(
                        f"Row {row_no} field low_ext_score_2: supplied "
                        f"{row.get('low_ext_score_2')} conflicts with EXT_SOURCE_2-derived {expected_flag}."
                    )
            else:
                working.at[idx, "low_ext_score_2"] = expected_flag

        if not _is_missing(ext3):
            expected_flag = derive_low_ext_score_3(ext3)
            if "low_ext_score_3" in working.columns and not _is_missing(row.get("low_ext_score_3")):
                if int(float(row.get("low_ext_score_3"))) != expected_flag:
                    add_error(
                        f"Row {row_no} field low_ext_score_3: supplied "
                        f"{row.get('low_ext_score_3')} conflicts with EXT_SOURCE_3-derived {expected_flag}."
                    )
            else:
                working.at[idx, "low_ext_score_3"] = expected_flag

        if "CNT_CHILDREN" in working.columns and not _is_missing(row.get("CNT_CHILDREN")):
            expected_flag = derive_many_children(row.get("CNT_CHILDREN"))
            if "many_children" in working.columns and not _is_missing(row.get("many_children")):
                if int(float(row.get("many_children"))) != expected_flag:
                    add_error(
                        f"Row {row_no} field many_children: supplied "
                        f"{row.get('many_children')} conflicts with CNT_CHILDREN-derived {expected_flag}."
                    )
            else:
                working.at[idx, "many_children"] = expected_flag

        if "credit_inquiries_year" in working.columns and not _is_missing(
            row.get("credit_inquiries_year")
        ):
            expected_flag = derive_high_inquiry_flag(row.get("credit_inquiries_year"))
        ...
        income_type = row.get("NAME_INCOME_TYPE") if "NAME_INCOME_TYPE" in working.columns else None
        age = row.get("age_years")
        emp = row.get("employed_years") if "employed_years" in working.columns else None
        if income_type is not None and not _is_missing(age):
            # employed_years may be missing for no-active tenure types
            ...
            # Only enforce when employment is deterministically resolved from income type
            # or an explicit tenure was supplied / derived.
    ...
    # Recount is approximate when capped; keep total_error_count as collected.
    if errors:
        if total_errors >= MAX_ERRORS:
            notices.append(
                f"Showing first {MAX_ERRORS} validation errors; additional errors may exist."
            )
        return BatchValidationResult(
            ok=False,
            errors=errors,
